...
void render2(std::vector<Point3D> points, std::vector<Edge> edges)
{
    ...
    // Update FL value for pulsing animation
    FL += 0.1 * DeltaTime; // Adjust pulsing speed as needed

    // Ensure FL remains within a reasonable range
    if (FL > 10) FL = 10; // Max scale out
    if (FL < 5) FL = 5;   // Min scale in
...
int main(int argc, char* argv[])
{
    ...
    while (running)
    {
        // Calculate DeltaTime for smooth animation (not implemented in this example)
        // You can use SDL_GetTicks() to calculate DeltaTime
